...
{
    // gcs_send_text_P(SEVERITY_LOW, PSTR("Calibrating barometer")); TODO:uncomment
    barometer.calibrate();
    // gcs_send_text_P(SEVERITY_LOW, PSTR("barometer calibration complete")); TODO:uncomment
}
...
static void read_battery(void)
{
    ...
    if (!ap.low_battery && ( battery_voltage1 < g.low_voltage || (g.battery_monitoring == BATT_MONITOR_VOLTAGE_AND_CURRENT && current_total1 > g.pack_capacity))) {
        ...
        if( low_battery_counter >= BATTERY_FS_COUNTER ) {
            ...
            // low_battery_event(); TODO:uncomment
        }
    }else{
