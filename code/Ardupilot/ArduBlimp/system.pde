...
{
    // gcs_send_text_P(SEVERITY_LOW,PSTR("GROUND START")); TODO:uncomment
...
static void set_mode(uint8_t mode)
{
    ...
    if(ap.manual_attitude) {
        ...
        // reset_nav_params(); TODO:uncomment
    }
...
//        if(control_mode <= ACRO && g.rc_3.control_in == 0 && !ap.failsafe_radio) { // JBW - commented out if statement with ACRO mode
            if(control_mode == STABILIZE && g.rc_3.control_in == 0 && !ap.failsafe_radio) {
            set_auto_armed(false);
        }
    }else{
        // arm checks
